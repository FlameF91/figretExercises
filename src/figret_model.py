...
class Figret():
    ...
    def __init__(self, props, env, device):
        ...
        self.commodities_to_paths = torch.sparse_coo_tensor(np.vstack((ctp_coo.row, ctp_coo.col)), \
                                                            ...
                                                            torch.Size(ctp_coo.shape)).to(
            device)  # shape: (num_commodities, num_paths)
        ...
        self.paths_to_edges = torch.sparse_coo_tensor(np.vstack((pte_coo.row, pte_coo.col)), \
                                                      ...
                                                      torch.Size(pte_coo.shape)).to(
            device)  # shape: (num_paths, num_edges)
        self.tm_hist_std = torch.tensor(env.simulator.get_tm_histories_std()).to(
            device)  # shape: (num_nodes * (num_nodes - 1),)
    ...
    def loss(self, y_pred_batch, y_true_batch):
        ...
        for i in range(batch_size):
            ...
            if self.env.constant_pathlen:
                max_sensitivity = torch.max(split_ratios.view(num_nodes * (num_nodes - 1), -1),
                                            dim=1).values  # shape: (num_commodities,)
            else:
                split_ratios = torch.split(split_ratios, self.env.commodities_to_path_nums)
                max_sensitivity = torch.tensor([torch.max(split_ratio) for split_ratio in split_ratios]).to(
                    self.device)  # shape: (num_commodities,)
    ...
    def test(self, test_dl, model, device):
        ...
        if len(test_losses) > 0:
            avg_ratio = sum(test_losses) / len(test_losses)
            max_ratio = max(test_losses)
            min_ratio = min(test_losses)

            print("\n" + "=" * 60)
            print(f"   FINAL EVALUATION RESULTS ({self.props.topo_name})")
            print("=" * 60)
            print(f"Mean Approximation Ratio: {avg_ratio:.5f} (Target: Close to 1.0)")
            print(f"Max (Worst) Ratio:        {max_ratio:.5f}")
            print(f"Min (Best) Ratio:         {min_ratio:.5f}")
